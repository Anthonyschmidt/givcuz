jQuery(function($) {"use strict";
	var Site = {
		...
		validateForm : function() {
			...
			function IsEmail(email) {
				...
				if (!regex.test(email)) {
					...
					$('#inputEmail').parents('.form-group').removeClass('error')
				}
			}
		...
		validation : function() {
			...
			$('#inputName,#inputSubject,#inputEmail,#textArea').blur(function() {
				validateForm2(this);
			});
			...
			$('#submit').click(function() {
				...
				if (x == null || x == "" || x == "Name") {
					...
					$('#inputName').closest('.form-group').addClass('error')
					bool = false;

				} else {
					i++;
					$('#inputName').closest('.form-group').removeClass('error');
					name_val = $('#inputName').val();

				}

				var x = $('#inputSubject').val();
				
				if (x == null || x == "" || x == "Name") {
					$('#inputSubject').closest('.form-group').addClass('error')
					bool = false;

				} else {
					i++;
					$('#inputSubject').closest('.form-group').removeClass('error');
					comp_val = $('#inputSubject').val();
				...
				if (atpos < 1 || dotpos < atpos + 2 || dotpos + 2 >= x.length || x == 'Email') {
					$('#inputEmail').closest('.form-group').addClass('error')
					bool = false;
				} else {

					i++;
					$('#inputEmail').closest('.form-group').removeClass('error');
					email_val = $('#inputEmail').val();

				}
				
				msg_val = $('#textArea').val();
